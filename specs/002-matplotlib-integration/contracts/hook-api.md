# Contract: `catplotlib.render.hook`

## `install() -> None`

Idempotent. Safe to call more than once — subsequent calls are no-ops (FR-009). Called exactly
once, from `catplotlib/__init__.py`, at import time. Never raises (FR-008) — any error during
...
| `Config.enabled` | `fig._catplotlib_decorated` | Behavior |
|---|---|---|
| `False` | any | Call original `draw()` only. No other catplotlib code executes. |
...
## `catplotlib.render.bboxes.extract_exclusions(figure) -> list[Rect]`
...
## `catplotlib.render.artist.draw_placements(figure, placements) -> None`
...
- `import catplotlib` → `install()` called once, never raises, works under `Agg`.
...
- `catplotlib.disable()` → all subsequent draws produce byte-identical output to matplotlib
  without catplotlib imported, for figures that have not yet been decorated. (Figures already
  decorated before `disable()` keep their existing cats — see spec Assumptions.)
- `catplotlib.enable()` after `disable()` → subsequent new figures decorate normally again.
